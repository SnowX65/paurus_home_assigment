...
| POST        | `/calculatePossibleBetReturnAmount` | Uses trader's country tax rules to compute best return method | `traderId` (Long) <br><br>  `playedAmount` (Double)<br><br>  `odd` (Double)<br> |  Returns: <br><br> `possibleReturnAmountBefTax`  best possible return before the tax is calculated <br><br> `taxRate`  the tax rate the trader uses<br><br> `taxAmount`  the tax amount the traders uses<br><br> `taxMethod` - `(amount/rate)`  which method the trader chose when calculating the possible return after tax    |
